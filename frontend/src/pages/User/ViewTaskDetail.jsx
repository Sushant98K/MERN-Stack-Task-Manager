    ...
    if (todoChecklist && todoChecklist[index]) {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          // Optionally revert the toggle if the API calls fails
          todoChecklist[index].completed = !todoChecklist[index].completed
        }
      } catch (error) {
        todoChecklist[index].completed = !todoChecklist[index].completed
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 mt-4">
            <div className="form-card col-span-3">
              <div className="flex items-center justify-between">
                ...
                <div
                  className={`text-[11px] md:text-[13px] font-bold ${getStatusTagColor(
                    task?.status
                  )} px-4 py-0.5 rounded`}
                >
                  {task?.status}
...
const InfoBox = ({ label, value }) => {
  return (
    <>
      ...
      <p className="text-[12px] md:text-[13px] font-bold text-gray-700 mt-0.5">
        {value}
      </p>
    </>
  );
...
const TodoCheckList = ({ text, isChecked, onChange }) => {
  return (
    <div className="flex items-center gap-3 p-3">
      ...
      <p className="text-[13px] font-bold text-gray-800">{text}</p>
    </div>
  );
};
...
const Attachment = ({ link, index, onClick }) => {
  return (
    <div
      ...
      onClick={onClick}
    >
      <div className="flex-1 flex temcent  gap-3">
